...
TextSpan _qpcV4SpanSegment({
  ...
  if (!wordInfoCtrl.isWordSelectionEnabled) {
    // تحديد الكلمة معطّل: الضغط القصير يخفي/يظهر عناصر التحكم، الضغط المطوّل يفتح قائمة الآية
    recognizer = TapLongPressRecognizer(
      ...
      ..onQuickTapCallback = _toggleControls
    ...
    recognizer = TapLongPressRecognizer(
      ...
      ..onQuickTapCallback = () {
        // Clear active word selection on tap-away; otherwise toggle controls.
